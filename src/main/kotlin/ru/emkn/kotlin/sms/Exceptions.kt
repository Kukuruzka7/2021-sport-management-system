...
class ResultMissesAthleteNumber(fileName: String): Exception("В файле $fileName отсутствует номер спортсмена на первой строке")

class ResultByAthleteInvalidRow(fileName: String, line: List<String>): Exception("В файле $fileName содержит некорректную строку. Ожидается \n${InputAthleteResults.ATHLETE_RESULT_FORMAT}\n В реальность\n $line")